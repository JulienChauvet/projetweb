...
class Events extends Model
{
    ...
    protected $fillable = ['Suggest','Title','Description','DateEvent','PictureLink','Recurrence', 'MonthEvent'];
    ...
    public function get_MonthEvent() {
        $monthEvent = Events::where('MonthEvent',1)->first();
        if (empty($monthEvent)){
            $monthEvent = Events::where('DateEvent','>',Carbon::now()->toDateTimeString())->where('Suggest', 0)->first();
            }
            return $monthEvent;
        }
